...
import static net.sf.saxon.lib.FeatureKeys.*;
 ...
 *
 * <p>This class is not thread safe, because it uses {@link XPathExpression} and {@link
 * Transformer}. Make sure the methods are not called by multiple threads at the same time.
 *
 * @author R.W.Majeed
 */
public class CDAParser {
   ...
   *
   * <p>First element will select the patient id, which will not change even for multiple visits.
   *
   * <p>Second will be the the visit id, which will not change with document updates (sometimes also
   * called the set id)
   *
   * <p>Third the document version which changes with every submission of the same document. This
   * can also be a time stamp.
   */
  ...
  private final Transformer domTransform;
  ...
  /**
   * Find the document id for a given CDA document. It should be globally unique for any different
   * document, but should be the same for an update/replacement of the same document.
   *
   * @param cda CDA document
   * @return template id
   * @throws XPathExpressionException XPath error
